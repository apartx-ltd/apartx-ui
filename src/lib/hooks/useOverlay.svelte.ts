...
export function useOverlay(
  getOpen: () => boolean,
  close: () => void,
  opts?: { respectBack?: boolean; exitMs?: number; onBackRequest?: () => boolean },
): UseOverlay {
  ...
  $effect(() => {
    ...
    if (open && handle === null) {
      ...
      handle = registerOverlay({ close, scrim: true, exitMs: opts?.exitMs, beforeBackClose: opts?.onBackRequest });
      committed = false;
